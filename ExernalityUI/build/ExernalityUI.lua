...
function Window:Create(data)
	...
	local outline = U:Create("Frame", {Name = "ExernalityOutline", Position = UDim2.new(0.19056724, 0, 0.20240964, 0), Size = UDim2.new(0, T.WindowWidth, 0, T.WindowHeight), BackgroundTransparency = 1, BackgroundColor3 = scheme.bg, BorderSizePixel = 0, ClipsDescendants = false, ZIndex = 2, Parent = gui})
	U:CreateStroke(outline, Color3.fromRGB(138, 138, 138), 1, Enum.BorderStrokePosition.Outer); U:CreateCorner(outline)
	local mainFrame = U:Create("Frame", {Name = "Exernality", Position = UDim2.new(0.19056724, 0, 0.20240964, 0), Size = UDim2.new(0, T.WindowWidth, 0, T.WindowHeight), BackgroundColor3 = scheme.bg, BackgroundTransparency = 0, BorderSizePixel = 0, ClipsDescendants = false, ZIndex = 1, Parent = gui})
	...
	local lineH = U:Create("Frame", {Name = "Line", Position = UDim2.new(0, 0, 0, 0), Size = UDim2.new(0, T.WindowWidth, 0, T.TopbarHeight), BackgroundTransparency = 1, BorderSizePixel = 0, ClipsDescendants = false, ZIndex = 15, Parent = mainFrame})
	...
	local tabsFrame = U:Create("Frame", {Name = "Tabs", Position = UDim2.new(0.153608248, 0, 0.0770751014, 0), Size = UDim2.new(0, T.ContentWidth, 0, T.ContentHeight), BackgroundTransparency = 1, BorderSizePixel = 0, ClipsDescendants = false, Parent = mainFrame})
	self.Gui = gui; self.Outline = outline; self.Main = mainFrame; self.Logo = logo; self.LineH = lineH; self.LineV = lineV; self.TabButtons = tabButtons; self.TabsFrame = tabsFrame; self.TitleLabel = titleLabel; self.VersionLabel = versionLabel
...
function Window:Toggle()
	self.Visible = not self.Visible; self.Main.Visible = self.Visible; self.Outline.Visible = self.Visible
end
